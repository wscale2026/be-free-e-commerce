...
function FAQItem({
  ...
  return (
    <div
      className={`bg-white dark:bg-zinc-900 rounded-2xl border border-gray-200 dark:border-zinc-800 overflow-hidden shadow-sm transition-all duration-300 ${
        visible ? "animate-fade-in-up" : "opacity-0"
      } ${isOpen ? "shadow-card dark:shadow-zinc-950" : ""}`}
    ...
    >
      ...
      >
        <span className="text-base md:text-lg font-bold text-brand-dark dark:text-white">
          {faq.question}
        </span>
        <div
          className={`w-8 h-8 rounded-full flex items-center justify-center bg-brand-gray dark:bg-zinc-800 flex-shrink-0 transition-transform duration-300 ${
            isOpen ? "rotate-180" : ""
          }`}
        >
          <ChevronDown className="w-4 h-4 text-brand-dark dark:text-white" />
        </div>
      </button>
      <div
